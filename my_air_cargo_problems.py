...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        neg = decoded_state.neg

        for a in self.actions_list:
            if set(a.precond_pos).issubset(pos) and set(a.precond_neg).issubset(neg):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        """
        Every action becomes applicable in every state, and any single goal fluent can be achieved in one step (if 
        there is an applicable action — if not, the problem is impossible). This almost implies that the number of 
        steps required to solve the relaxed problem is the number of unsatisfied goals — almost but not quite, 
        because 
